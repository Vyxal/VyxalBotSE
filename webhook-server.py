...
def receive_github_webhook():
  ...
  if data.get("action") in ["opened", "reopened"] and "pull_request" in data:
    ...
    send(f"[**{data['release']['name'] or data['release']['tag_name']}**]({data['release']['html_url']})" + ("" if data["repository"]["full_name"] == "Vyxal/Vyxal" else " for " + data["repository"]["full_name"]), pin = data["repository"]["full_name"] == "Vyxal/Vyxal")
  return ""
