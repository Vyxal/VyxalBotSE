...
def receive_message():
    ...
    if match:
        ...
        if len(output) == 1 and len(output[0]) < 450:
            ...
            output.insert(
                0,
                f"[@{message['user_name'].replace(' ', '')}: {message['message_id']}]",
            )
            return "\n".join("    " + line for line in output)
    without_ping = re.sub("^" + ping_regex, "", content.lower()).strip()
    if re.match(r"^(exec(ute)?|run|run code|eval(uate)?)", without_ping):
        return (
            f"{reply} Did you forget to put backticks around your code (%s)? Remember to escape any backticks in your code (to type %s, enter %s)."
            % (r"`\`code\``", r"`\`hi\``", "`\`\\\`hi\\\`\``")
        )
    if re.match(
        r"^(status|((lol )?(yo)?u good( (there )?(my )?(epic )?(bro|dude|sis|buddy|mate|m8|gamer)?)?\??))$",
        without_ping,
    ):
    ...
    if re.match(
        r"^(please|pl[sz]) (run this vyxal code|gi(ve|b) lyxal link$)", without_ping
    ):
    ...
    if re.match(r"^(hyper-?ping|ping every(body|one))$", without_ping):
        if STORAGE["pings"]:
            return (
                " ".join(
                    "@" + x.replace(" ", "") for x in sorted(set(STORAGE["pings"]))
                )
                + " ^"
            )
        else:
    ...
    match = re.match(
        r"^issue\s+((.+?)\s+)?<b>(.+?)</b>\s*(.*?)(\s+<code>.+?</code>)+$", without_ping
    )
    if match:
        ...
        r = requests.post(
            ...
            data=json.dumps({"title": title, "body": body, "labels": tags}),
        )
...
def receive_repository_webhook():
    ...
    if action == "created":
        ...
        if repo["full_name"] == "Vyxal/Vyxal":
            send(
                "@lyxal @AaronMiller @Razetime @user @Ausername @hyper-neutrino **NOTICE**: Potential catastrophic failure. It appears that the Vyxal/Vyxal repository has been deleted."
            )
    elif action == "archived":
        send(f"{link(user)} archived {linkrepo(repo)}")
        if repo["full_name"] == "Vyxal/Vyxal":
            send(
                "@lyxal @hyper-neutrino It appears that the Vyxal/Vyxal repository was archived. Please review recent activity."
            )
        ...
        if repo["full_name"] == "Vyxal/Vyxal":
            send(
                "@lyxal It appears that the Vyxal/Vyxal repository was privatized. Please reivew this change."
            )
    return ""
...
def receive_github_webhook():
    ...
    if "forkee" in data:
        fork = data["forkee"]
        send(
            f"{link(data['sender']['login'])} forked {linkrepo(data['repository'])} into {linkrepo(fork)}"
        )
    if "issue" in data:
        issue = data["issue"]
        if data["action"] == "opened":
            send(
                f"{link(data['sender']['login'])} opened {linkissue(issue, False)} in {linkrepo(data['repository'])}: _{msgify(issue['title'])}_"
            )
        elif data["action"] == "reopened":
            send(
                f"{linkissue(issue)} ({linkrepo(data['repository'])}) was reopened by {link(data['sender']['login'])}"
            )
        elif data["action"] == "closed":
            send(
                f"{linkissue(issue)} ({linkrepo(data['repository'])}) was closed by {link(data['sender']['login'])}"
            )
        elif data["action"] == "edited":
            send(
                f"{linkissue(issue)} ({linkrepo(data['repository'])}) was edited by {link(data['sender']['login'])}"
            )
    if data.get("action") in ["opened", "reopened"] and "pull_request" in data:
        pr = data["pull_request"]
        send(
            f"[PR #{data['number']}]({pr['html_url']}) ({linkrepo(data['repository'])}) {data['action']} by {link(data['sender']['login'])} from {pr['head']['label']} into {pr['base']['label']}: _{msgify(pr['title'])}_"
        )
    elif data.get("action") == "closed" and "pull_request" in data:
        pr = data["pull_request"]
        if pr["merged"]:
            send(
                f"[PR #{data['number']}]({pr['html_url']}) ({linkrepo(data['repository'])}) was merged by {link(data['sender']['login'])} from {pr['head']['label']} into {pr['base']['label']}: _{msgify(pr['title'])}_"
            )
        else:
            send(
                f"[PR #{data['number']}]({pr['html_url']}) ({linkrepo(data['repository'])}) was closed by {link(data['sender']['login'])} with unmerged commits."
            )
        ...
        if "commits" in data:
            for commit in data["commits"]:
                send(
                    f"{link(data['sender']['login'])} pushed a [commit]({commit['url']}) to {linkref(refname, data)}: _{msgify(commit['message'])}_"
                )
        if data["created"]:
            oldref = data["base_ref"][11:]
            send(
                f"{link(data['sender']['login'])} created branch {linkref(refname, data)} from {linkref(oldref, data)}"
            )
        elif data["deleted"]:
            send(
                f"{link(data['sender']['login'])} deleted branch {data['repository']['name']}/{refname}"
            )
    elif data.get("action") == "create" and "alert" in data:
        alert = data["alert"]
        send(
            f"**security alert** ({alert['severity']}) created by {link(data['sender']['login'])} in {linkrepo(data['repository'])}) (affected package: _{msgify(alert['affected_package_name'])} {alert['affected_range']})"
        )
    elif data.get("action") == "published" and "release" in data:
        send(
            f"[**{data['release']['name'] or data['release']['tag_name']}**]({data['release']['html_url']})",
            pin=data["repository"]["full_name"] == "Vyxal/Vyxal",
        )
    return ""
