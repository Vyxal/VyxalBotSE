...
def receive_message():
  ...
  if re.match("^" + ping_regex, content.lower()):
    ...
    if re.match(r"^blame$", without_ping):
      return f"{reply} It was {random.choice({'wasif', 'Underslash', 'math', 'Aaron Miller', 'A username', 'user', 'Unrelated String', 'AviFS', 'Razetime', 'lyxal', '2x-1', 'hyper-neutrino'} - {message['user_name']})}'s fault!"
